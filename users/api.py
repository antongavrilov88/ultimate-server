...
from .commands.exceptions import AccessDeniedError, UserAuthorizationError, GetUsersListError, \
...
users_error: object = api.model('users_error', {
    'message': fields.String(readonly=True)
})


@api.errorhandler(AccessDeniedError)
@api.marshal_with(users_error, code=403, description="Forbidden.")
def handle_email_conflict_exception(error):
    """This is access error"""
    return {'message': error.message}, 403


@api.errorhandler(UserDoesNotExistenceError)
@api.marshal_with(users_error, code=404, description="User does not exist.")
def handle_email_conflict_exception(error):
    """User does not exist"""
...
@api.marshal_with(users_error, code=401, description="Unauthorized")
def handle_email_conflict_exception(error):
...
@api.marshal_with(users_error, code=500, description="Request failed")
def handle_entity_processing_exception(error):
    """This is an unknown error"""
    return {'message': error.message}, 500


@api.errorhandler(UnknownUserRequestError)
@api.marshal_with(users_error, code=500, description="Internal server error")
